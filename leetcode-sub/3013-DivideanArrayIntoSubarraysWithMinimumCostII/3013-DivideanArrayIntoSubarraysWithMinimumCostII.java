...
class Solution {
    ...
    public int countSteppingNumbers(String low, String high) {
        ...
        Long[][][][] dp1 = new Long[m][2][11][2];
        long res1 = solve(a, m, 0, 1, -1, 1, dp1);
        return (int) ((res2 - res1 + mod) % mod);
    }
        private long solve(String num, int m, int idx, int tight, int prev,
        int leadingZero, Long[][][][] dp) {
        if (idx == m) {
            return leadingZero == 0 ? 1 : 0;
        }
        if (dp[idx][tight][prev + 1][leadingZero] != null) {
        ...
        for (int i = 0; i <= limit; i++) {
            ...
            if (leadingZero == 1 || prev == -1 || Math.abs(i - prev) == 1) {
                count = (count + solve(num, m, idx + 1, newTight, i,
                newLeadingZero, dp)) % mod;
            }
        }
