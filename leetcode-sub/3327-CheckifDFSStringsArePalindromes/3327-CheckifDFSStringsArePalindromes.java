// Last updated: 3/7/2026, 7:55:40 PM
1class Solution {
2    int idx = 0;
...
18            res[i] = isPalindrome(range[i][0], range[i][1], p);
19        }
20        return res;
21    }
22    private void dfs(int node, List<List<Integer>> adj, 
23    char[] s, char[] dfsStr, int[][] range) {
24        range[node][0] = idx;
25        for (int child : adj.get(node)) {
26            dfs(child, adj, s, dfsStr, range);
27        }
28        dfsStr[idx] = s[node];
29        range[node][1] = idx;
30        idx++;
31    }
