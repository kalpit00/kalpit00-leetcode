// Last updated: 5/22/2026, 7:01:28 AM
...
37        while (right < n) {
38            sum += nums[right];
39            windowSum += nums[right] * (right - left + 1);
40            right++;
41            while (sum > target) {
42                windowSum -= sum;
43                sum -= nums[left];
44                left++;
45            } // count subarrays with sum <= target
46            count += right - left;
47            total += windowSum;
48        } // total = total sum of all subarrays with sum <= target
49        return new long[]{count, total};
50    }
51}
52